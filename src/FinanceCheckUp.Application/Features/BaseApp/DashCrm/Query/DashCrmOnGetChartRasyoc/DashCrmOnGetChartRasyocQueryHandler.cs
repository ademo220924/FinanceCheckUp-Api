    ...
    IRasyoAnalizMainManager rasyoAnalizMainManager) : IRequestHandler<DashCrmOnGetChartRasyocQuery, GenericResult<DashCrmOnGetChartRasyocResponse>>
{
    ...
    public async Task<GenericResult<DashCrmOnGetChartRasyocResponse>> Handle(DashCrmOnGetChartRasyocQuery request, CancellationToken cancellationToken)
    {
        ...
        request.Request.InitialModel.RasyoAnalizViewCrm.SetResult(request.Request.InitialModel.RasyoAnalizCRM, request.Request.InitialModel.CurrentUser.SelectedYear);
                return GenericResult<DashCrmOnGetChartRasyocResponse>.Success(new DashCrmOnGetChartRasyocResponse { Response = DataSourceLoader.Load(request.Request.InitialModel.RasyoAnalizViewCrm.nresult.OrderByDescending(z => z.Value), request.Request.Options) });
    }
}