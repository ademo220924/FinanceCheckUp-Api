...
public class GetListByCompanyAndAccountIdQueryHandle(
    ...
    IGenericRepository<ReminderAccountGroup, long> reminderAccountGroupRepository) : IRequestHandler<GetListByCompanyAndAccountIdQuery, GenericResult<List<ReminderRuleJobDto>>>
{
    ...
    public async Task<GenericResult<List<ReminderRuleJobDto>>> Handle(GetListByCompanyAndAccountIdQuery request, CancellationToken cancellationToken)
    {
        var mappingModel = ManuelMappingUtilities.SetMapping(await _reminderRuleJobRepository.GetFilterAsync(request.Model.CompanyId, request.Model.AccountId),
