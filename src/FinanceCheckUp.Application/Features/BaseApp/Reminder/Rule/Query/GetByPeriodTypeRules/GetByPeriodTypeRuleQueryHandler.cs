...
public class GetByPeriodTypeRuleQueryHandle(
    IGenericRepository<ReminderRule, long> reminderRuleRepository,
    IMapper mapper,
    IGenericRepository<ReminderAccount, long> accountRepository) : IRequestHandler<GetByPeriodTypeRuleQuery, GenericResult<List<RemainderRuleDto>>>
{
    ...
    public async Task<GenericResult<List<RemainderRuleDto>>> Handle(GetByPeriodTypeRuleQuery request, CancellationToken cancellationToken)
    {
        var list = await _reminderRuleRepository.GetListAsync(c => c.PeriodType.Equals(request.PeriodType), cancellationToken);
        if (list.Count == 0)
        ...
        if (mainAccounts == null || mainAccounts.Count == 0)
            throw new NotFoundException(nameof(ReminderAccount), request.PeriodType.ToString());
