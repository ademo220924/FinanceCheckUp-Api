...
public class GetListByAccountGroupIdQueryHandle(
    ...
    IMapper mapper)
    : IRequestHandler<GetListByAccountGroupIdQuery, GenericResult<List<ReminderAccountDto>>>
{
    ...
    public async Task<GenericResult<List<ReminderAccountDto>>> Handle(GetListByAccountGroupIdQuery request, CancellationToken cancellationToken)
    {
        var accounts = await _reminderAccountRepository.GetListAsync(c => c.AccountGroupId.Equals(request.AccountGroupId), cancellationToken);
