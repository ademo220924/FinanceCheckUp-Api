...
public class GetListAccountQueryHandle(
    ...
    IMapper mapper)
    : IRequestHandler<GetListAccountQuery, GenericResult<List<ReminderAccountDto>>>
{
    ...
    public async Task<GenericResult<List<ReminderAccountDto>>> Handle(GetListAccountQuery request, CancellationToken cancellationToken)
    {
        var accounts = await _reminderAccountRepository.GetListAsync(cancellationToken: cancellationToken);
