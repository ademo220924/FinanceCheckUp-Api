...
public class MizanDashBilancoRevenueAktOnGetQueryHandler(IDashGelirTablosuManager dashGelirTablosuManager,IHhvnUsersManager hhvnUsersManager, ICompanyManager companiesManager, IUserTypeManager userTypeManager, ICompanyManager companyManager, ITBLXmlManager tBLXmlManager) : IRequestHandler<MizanDashBilancoRevenueAktOnGetQuery, GenericResult<MizanDashBilancoRevenueAktOnGetResponse>>
{
    public Task<GenericResult<MizanDashBilancoRevenueAktOnGetResponse>> Handle(MizanDashBilancoRevenueAktOnGetQuery request, CancellationToken cancellationToken)
    {
        var userId = Convert.ToInt64(request.UserId);
        var responseModel = new MizanDashBilancoRevenueAktRequestInitialModel
        {
            UserID = userId,
        ...
        responseModel.nRequestList = dashGelirTablosuManager.Get_MAINRevenueMznAkt(request.Request.myear, responseModel.CompID);
        ...
        return Task.FromResult(GenericResult<MizanDashBilancoRevenueAktOnGetResponse>.Success(
            new MizanDashBilancoRevenueAktOnGetResponse
            {
