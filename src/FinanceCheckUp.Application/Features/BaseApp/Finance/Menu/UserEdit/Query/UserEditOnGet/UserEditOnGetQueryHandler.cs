...
public class UserEditOnGetQueryHandler(IHhvnUsersManager hhvnUsersManager, IUserTypeManager userTypeManager, ICompanyManager companyManager, ITBLXmlManager tBLXmlManager) : IRequestHandler<UserEditOnGetQuery, GenericResult<UserEditOnGetResponse>>
{

    public Task<GenericResult<UserEditOnGetResponse>> Handle(UserEditOnGetQuery request, CancellationToken cancellationToken)
    {
        var userId = Convert.ToInt64(request.UserId);
        var responseModel = new UserEditInitialModel
        {
            UserID = userId,
        ...
        return Task.FromResult(GenericResult<UserEditOnGetResponse>.Success(
            new UserEditOnGetResponse
            {
