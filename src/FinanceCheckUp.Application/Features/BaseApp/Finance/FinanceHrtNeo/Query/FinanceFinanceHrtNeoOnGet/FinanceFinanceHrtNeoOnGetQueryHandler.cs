...
public class FinanceFinanceHrtNeoOnGetQueryHandler(
    IHhvnUsersManager hhvnUsersManager, 
    ICompanyManager companiesManager,  
    ITBLXmlManager tBLXmlManager, 
    IDashGelirTablosuManager dashGelirTablosuManager) : IRequestHandler<FinanceFinanceHrtNeoOnGetQuery, GenericResult<FinanceFinanceHrtNeoOnGetResponse>>
{
    public Task<GenericResult<FinanceFinanceHrtNeoOnGetResponse>> Handle(FinanceFinanceHrtNeoOnGetQuery request, CancellationToken cancellationToken)
    {
        var userId = Convert.ToInt64(request.UserId);
        var responseModel = new FinanceFinanceHrtNeoRequestInitialModel
        {
            UserID = userId
        };
        ...
        return Task.FromResult(GenericResult<FinanceFinanceHrtNeoOnGetResponse>.Success(new FinanceFinanceHrtNeoOnGetResponse
        {
