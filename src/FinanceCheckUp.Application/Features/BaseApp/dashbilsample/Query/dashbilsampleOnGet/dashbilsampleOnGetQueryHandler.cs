...
public class dashbilsampleOnGetQueryHandler(ICompanyManager companyManager, IHhvnUsersManager hhvnUsersManager, ITBLXmlManager tBLXmlManager, IDashGelirTablosuManager dashGelirTablosuManager) : IRequestHandler<dashbilsampleOnGetQuery, GenericResult<dashbilsampleOnGetResponse>>
{

    public async Task<GenericResult<dashbilsampleOnGetResponse>> Handle(dashbilsampleOnGetQuery request, CancellationToken cancellationToken)
    {
        var UserID = Convert.ToInt64(request.UserId);
        var responseModel = new dashbilsampleRequest
        {
            UserID = UserID,
        ...
        responseModel.nRequestList = dashGelirTablosuManager.Get_MAINBilancoMznSmm(request.Request.myear, responseModel.CompID).Where(x => x.SmmAfter != 0 && x.SmmBefore != 0);
        ...
        return GenericResult<dashbilsampleOnGetResponse>.Success(new dashbilsampleOnGetResponse { InitialModel = responseModel });
    }
}