...
public class MoodUploadBeyannameUpdateCommandHandle(
    ...
    IDashRasyoManager dashRasyoManager) : IRequestHandler<MoodUploadBeyannameUpdateCommand, GenericResult<BeyannameMoodUploadUpdateResponse>>
{
    public async Task<GenericResult<BeyannameMoodUploadUpdateResponse>> Handle(MoodUploadBeyannameUpdateCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var fileItems = await fileOperationService.ReadPdfFileAsync(filePath, cancellationToken);
            List<ReadPdfPg> CHKgROUP = [.. fileItems.Data];
            ...
            List<ReadPdfPg> nliste = [];
            List<ReadPdfPg> nliste1 = [];
            ...
            if (chhhk == null && chhhkt == null && chhhkt1 == null)
            {
                ...
                return GenericResult<BeyannameMoodUploadUpdateResponse>.Fail("Hatalı PDF Yükleme - KURUMLAR VERGİSİ BEYANNAMESİ - GEÇİCİ VERGİ BEYANNAMESİ - YILLIK GELİR VERGİSİ BEYANNAMESİ Olmalı");
            }
            ...
            if (chhhkt != null)
            {
                ...
                return GenericResult<BeyannameMoodUploadUpdateResponse>.Fail("Hatalı PDF Yükleme - Yalnızca  KURUMLAR VERGİSİ BEYANNAMESİ Bu alandan Yüklenebilir");
            }
            ...
            if (ISNoAdmin)
            {
                if (vergino.Trim() != mainComp.TaxId.Trim())
                {
                    ...
                    return GenericResult<BeyannameMoodUploadUpdateResponse>.Fail("Hatalı Vergi No");
                }
            }
            ...
            if (chkyil1 != nYear.ToString())
            {
                if (chkyil3 != nYear.ToString())
                {
                    ...
                    if (Convert.ToInt32(txt1Yil) != nYear)
                    {
                        ...
                        return GenericResult<BeyannameMoodUploadUpdateResponse>.Fail("Hatalı Yıl");
                    }
            ...
            for (int i = 0; i < CHKgROUP.Count; i++)
            {
                ...
                if (chekSource)
                {
                    checkval = CHKgROUP[i].LineContent.Length > 1 ? CHKgROUP[i].LineContent.Replace(". ", string.Empty)[..1] : string.Empty;
            ...
            nliste=nliste.Select(c => { c.IsGecici = 0; return c; }).ToList();

            BeyannameChk btnchk = new();
            beyannameChkManager.Delete(CompID, nYear);
            if (ISGeciciVergi)
            {
                nliste = nliste.Select(c => { c.IsGecici = 1; return c; }).ToList();
            ...
            if (!ISGeciciVergi)
            {
                ...
                {
                    return GenericResult<BeyannameMoodUploadUpdateResponse>.Success(new BeyannameMoodUploadUpdateResponse { ResponseText = new JsonResult("ok") });
                }
                ...
                if (WCapitalViea.Count > 0)
                {
                    ...
                    dashRasyoMizanManager.GetDashOzetMali(nYear - 1, CompID);
                }
            }
        }
        catch (Exception ex)
        {
            ...
            return GenericResult<BeyannameMoodUploadUpdateResponse>.Fail(ex.ToString());
