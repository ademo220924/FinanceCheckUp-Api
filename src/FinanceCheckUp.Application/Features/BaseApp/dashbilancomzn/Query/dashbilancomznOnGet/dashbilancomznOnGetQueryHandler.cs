...
public class dashbilancomznOnGetQueryHandler(ICompanyManager companyManager, IHhvnUsersManager hhvnUsersManager, ITBLXmlManager tBLXmlManager, IDashGelirTablosuManager dashGelirTablosuManager) : IRequestHandler<dashbilancomznOnGetQuery, GenericResult<dashbilancomznOnGetResponse>>
{
    public async Task<GenericResult<dashbilancomznOnGetResponse>> Handle(dashbilancomznOnGetQuery request, CancellationToken cancellationToken)
    {
        var UserID = Convert.ToInt64(request.UserId);
        var responseModel = new dashbilancomznRequest
        {
            UserID = UserID,
        ...
        responseModel.nRequestList = dashGelirTablosuManager.Get_MAINBilancoMznSmm(request.myear, responseModel.CompID).Where(x => x.SmmAfter != 0 && x.SmmBefore != 0);
        ...
        return GenericResult<dashbilancomznOnGetResponse>.Success(new dashbilancomznOnGetResponse { InitialModel = responseModel });
    }
}