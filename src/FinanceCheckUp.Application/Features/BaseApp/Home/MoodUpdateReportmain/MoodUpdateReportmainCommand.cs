using FinanceCheckUp.Application.Models.Requests.Home;
using FinanceCheckUp.Application.Models.Responses.Home;
using FinanceCheckUp.Framework.Core.Models;
using MediatR;

namespace FinanceCheckUp.Application.Features.BaseApp.Home.MoodUpdateReportmain;

public class MoodUpdateReportmainCommand: IRequest<GenericResult<MoodUpdateReportmainResponse>>
{
    public MoodUpdateReportmainRequest MoodUpdateReportmainRequest { get; set; }
}
