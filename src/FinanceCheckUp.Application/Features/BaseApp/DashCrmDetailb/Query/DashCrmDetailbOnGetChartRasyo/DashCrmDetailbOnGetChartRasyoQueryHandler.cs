    ...
    IRasyoAnalizMainManager rasyoAnalizMainManager) : IRequestHandler<DashCrmDetailaOnGetChartRasyoQuery, GenericResult<DashCrmDetailaOnGetChartRasyoResponse>>
{
    ...
    public async Task<GenericResult<DashCrmDetailaOnGetChartRasyoResponse>> Handle(DashCrmDetailaOnGetChartRasyoQuery request, CancellationToken cancellationToken)
    {
        ...
        request.Request.InitialModel.RasyoAnalizCRM = rasyoAnalizMainManager.CRMAnalizTOTAL120T(request.Request.InitialModel.CurrentUser.SelectedYear, request.Request.InitialModel.CompID);
        ...
        return GenericResult<DashCrmDetailaOnGetChartRasyoResponse>.Success(new DashCrmDetailaOnGetChartRasyoResponse { Response = new JsonResult(DataSourceLoader.Load(request.Request.InitialModel.RasyoAnalizViewCrm.nresult.OrderByDescending(z => z.Value), request.Request.Options)) });
